	...
	public boolean[] doLogin(String userName, String inputPassword) {
		...
		try {
			...
			if (storedPasswordHash.isPresent() && checkPassword(inputPassword, storedPasswordHash.get())) {
				Optional<Long> userId = dbManager.getUserId(userName);
				if (userId.isPresent()) {
					boolean isAdmin = dbManager.isAdminUser(userId.get());
					UserSession.getInstance().setUserId(userId.get());
					System.out
							.println("Debug: User logged in successfully - UserID: " + userId.get() + ", Username: " + userName);
					result[0] = true;
					result[1] = isAdmin;
				}
			}
